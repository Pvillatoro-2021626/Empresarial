...
FECHA DE MODIFICACION: 27-01-2022 
*/

drop database if exists DBEmpresaMultinacionalIN5AV;
create database DBEmpresaMultinacionalIN5AV;

use DBEmpresaMultinacionalIN5AV;
...
select E.nombre, 2022 - year(fechaDeNacimiento) as Edad from Empleados E;

/*2. fecha de contratacion de cada empleado*/
...
/*4. numero de empleados que hay para cada una de las edades */
select 2022 - year(fechaDeNacimiento) as Edad, count(*) from Empleados group by 2022 - year(fechaDeNacimiento) 
	order by Edad asc;
...
select 
    ...
    E.categoriaId,
    E.seccionId
    from Empleados E where empleadoId = 101;
    
/*7. empleado del departamento X */
select empleadoid, nombre, fechaDeNacimiento, 2022 - year(fechaDeNacimiento) as Edad, telefonoPersonal,
		fechaDeContratacion, categoriaId from Departamentos D inner join Oficinas O 
			on D.departamentoId = O.departamentoId inner join Secciones S on O.oficinaId = S.oficinaId
				inner join Empleados E on S.seccionId = E.seccionId
					where D.departamento = 'El Progreso';
                    
/*8. empleado cuya contratacion se produjo en el año X */
select empleadoid, nombre, fechaDeNacimiento, 2022 - year(fechaDeNacimiento) as Edad, telefonoPersonal,
		fechaDeContratacion, categoriaId, seccionId from Empleados where year(fechaDeContratacion) = 2015;
        
/*9. empleados que no sean jefe de seccion de la categoria X*/
select empleadoId, nombre, fechaDeNacimiento, 2022 - year(fechaDeNacimiento) as Edad, telefonoPersonal,
	fechaDeContratacion, E.categoriaId, E.seccionId, nombreCategoria, nombreSeccion  
		from Categorias C inner join Empleados E on C.categoriaId = E.categoriaId
            ...
            where S.seccionId = 5001 and C.nombreCategoria != 'Jefe de seccion';
            
/*10. empleados de nombre Jose */
select empleadoId, nombre, fechaDeNacimiento, 2022 - year(fechaDeNacimiento) as Edad, telefonoPersonal,
